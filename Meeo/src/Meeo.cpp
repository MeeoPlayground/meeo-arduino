...
void MeeoCore::setDataReceivedHandler(void (* f)(String, String)) {
    this->_dataReceivedHandler = f;
    pubSubClient.setCallback(this->_callbackHandler);
}

void MeeoCore::_callbackHandler(char * topic, uint8_t * payload, unsigned int payloadLength) {
    String sTopic = this->convertToString(topic);
    String sPayload = this->convertToString(payload,payloadLength);
    this->_dataReceivedHandler(sTopic, sPayload);
}
